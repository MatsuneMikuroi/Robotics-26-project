...
class ArUcoCamera:
    ...
    def get_marker_positions(self, frame=None, draw=True):
        """
        ...
        Args:
            ...
            draw (bool): Whether to draw marker axes and annotations on the frame.
                         Set to False to get a clean frame with no overlays.

        Returns:
            tuple: (processed_frame, markers_dict)
                - processed_frame: The frame, with overlays if draw=True.
    ...
    def reconnect(self):
        """Releases and reopens the RTSP stream."""
        self.cap.release()
        self.cap = cv2.VideoCapture(self.rtsp_url)
